...
# Force utf-8 output on interpreters that default to another encoding
# (e.g. older Windows consoles). This only makes sense for real,
# detachable byte streams. On some hosts (e.g. Android/Chaquopy,
# which replaces sys.stdout/sys.stderr with a custom file-like object
# that only implements write()/flush()) neither detach() nor
# reconfigure() exist, so skip the rewrap there instead of crashing.
for _stream_name in ("stdout", "stderr"):
    ...
    try:
        if hasattr(_stream, "reconfigure"):
            ...
            setattr(sys, _stream_name, io.TextIOWrapper(_stream.detach(), encoding='utf-8'))
        # else: stream doesn't support re-encoding (e.g. a custom UI
        # stream) -- leave it alone, it already handles str just fine.
    except Exception:
        pass
