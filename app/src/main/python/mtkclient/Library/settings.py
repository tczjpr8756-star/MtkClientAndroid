...
class HwParam:
    ...
    def __init__(self, config, meid: str, path: str = "logs"):
        ...
        if meid is None:
            ...
            if os.path.exists(os.path.join(path, self.paramfile)):
                try:
                    with open(os.path.join(path, self.paramfile), "r", encoding="utf-8") as rf:
                        tmp = json.loads(rf.read())
                    if tmp["meid"] == meid:
    ...
    def write_json(self):
        if self.paramsetting is not None:
            ...
            with open(os.path.join(self.config.hwparam_path, self.paramfile), "w", encoding="utf-8") as wf:
                wf.write(json.dumps(self.paramsetting))
