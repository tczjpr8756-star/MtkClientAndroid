...
# pyusb (and its libusb backends) are a desktop-only dependency: there is no
# usable libusb build for Android, so the app's Chaquopy pip config never
# installs pyusb, and androidusblib.UsbClass is used instead (see Port.py).
# This module is still imported at class-definition time by a handful of
# Exploit/* modules (for `usb.core.USBError`, raw `usb.core.find`, etc.), so
# the import itself must never raise -- only code paths that actually touch
# `usb.*` at runtime should fail, and only on platforms without pyusb.
try:
...
class UsbClass(DeviceClass):
    ...
    def set_line_coding(self, baudrate=None, parity=0, databits=8, stopbits=1, isFtdi=False):
        ...
        wIndex = 1 if isFtdi else 0
        try:
            wlen = self.ctrl_transfer(
                req_type, CdcCmds.SET_LINE_CODING,
                data_or_w_length=data, w_index=wIndex)
