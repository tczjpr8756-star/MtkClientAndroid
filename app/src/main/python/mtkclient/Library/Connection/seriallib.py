...
# pyserial (and termios) are desktop-only: the Chaquopy pip config never
# installs pyserial, and Android's Chaquopy CPython build doesn't ship
# termios either. Port.py imports this module unconditionally (SerialClass
# is only actually instantiated when the user passes --serialport, a
# desktop-only feature), so -- same as usblib.py -- the import itself must
# never raise; only code paths that actually need serial I/O should fail,
# and only on platforms that lack it.
try:
    import serial
...
def _reset_input_buffer_org(self):
    if sys.platform != "win32" and termios is not None:
...
class SerialClass(DeviceClass):
    ...
    def connect(self, ep_in=-1, ep_out=-1):
        if not HAS_PYSERIAL:
            raise RuntimeError("pyserial is not available on this platform; "
                               "serial-port mode (--serialport) isn't supported here, "
                               "use the normal USB connection instead.")
        if self.connected:
            self.close()
